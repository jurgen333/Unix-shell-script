...
//declarations-
char history[10][BUFFER_SIZE]; //history 2D array to store history commands
int count = 0;
...
int formatCommand(char inputBuffer[], char *args[],int *flag)
{
   	int length; // # of chars in command line
    	...
    	int argIndex = 0; // index of where to place the next parameter into args[]
    	int hist;
    	//read user input on command line and checking whether the command is !! or !n
	
 	length = read(STDIN_FILENO, inputBuffer, MAX_LINE);//fill inputBuffer
   ...
   //examine each character
    for (i=0;i<length;i++)
    {
        switch (inputBuffer[i])
        {
            ...
            case '\t' :               // to seperate arguments
                if(start != -1)
                {
                    args[argIndex] = &inputBuffer[start];    
                    argIndex++;
                }
            ...
            case '\n':                 //final char 
                ...
                inputBuffer[i] = '\0';
                args[argIndex] = NULL; // no more args
                break;
    ...
    args[argIndex] = NULL; //if the input line was > 80
